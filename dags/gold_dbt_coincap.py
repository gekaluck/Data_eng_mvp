    ...
    params={
        ...
        "start_date": Param(
            ...
            description=(
                ...
                "Keeps dbt Gold in step with Spark Gold on a multi-day catch-up."
            ),
        ),
        "end_date": Param(
            default=None,
...
def gold_dbt_coincap_assets():
    ...
    def run_dbt_gold(**context):
        """Run dbt Gold models for each resolved target date.

        Mirrors Spark Gold: a single date by default, or an inclusive range so both
        Gold implementations cover the same days after a catch-up. dbt is invoked
        once per date because the models are parameterised by a single
        `snapshot_date` var.
        """
        ...
        logger.info(
            "dbt Gold build will run for %d date(s): %s",
            len(target_dates),
        ...
        for target_date in target_dates:
            ...
            if result.returncode != 0:
                logger.error(
                    "dbt Gold build failed for %s (exit code %d).",
                    target_date_str,
                    result.returncode,
                )
                failures.append(target_date_str)
        ...
        if failures:
            raise RuntimeError(
                f"dbt Gold build failed for {len(failures)} of {len(target_dates)} "

...
- Updated with MERGE INTO (upsert): new coins are inserted and stable metadata is refreshed
- Fields: id, symbol, name, supply, max_supply
...
The API also includes a `rank`, but that value changes over time and is better treated as
an observed metric or derived analytic than as static coin metadata. In this project,
rank is not persisted in Silver `coins`; Gold derives market-cap rank from daily snapshot data.
